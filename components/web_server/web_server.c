...
".wifi-list{max-height:300px;overflow-y:auto;border:1px solid #e0e0e0;"
"border-radius:12px;margin-bottom:16px}"
".wifi-item{padding:10px 16px;border-bottom:1px solid #f0f0f0;cursor:pointer;"
...
".btn-copy{background:none;border:1px solid #ddd;border-radius:6px;padding:3px 8px;"
"cursor:pointer;font-size:12px;color:#667eea;transition:all 0.2s;white-space:nowrap}"
...
"<script>""let selectedSSID='';"
"let scanTimer=null;"
"window.onload=function(){scanWiFi();scanTimer=setInterval(scanWiFi,30000);document.getElementById('scanAuto').textContent='\u81EA\u52A8\u5237\u65B0: 30\u79D2';};"
...
"ta.value=ssid;ta.style.position='fixed';ta.style.left='-9999px';"
"document.body.appendChild(ta);ta.select();"
"try{document.execCommand('copy');showCopied(btn);showToast('\u5DF2\u590D\u5236: '+ssid);}"
...
"if(data.length===0){list.innerHTML='<div style=\"padding:20px;text-align:center;color:#888\">\u672A\u53D1\u73B0Wi-Fi\u4FE1\u53F7</div>';return;}"
...
"var copyId='c'+Math.random().toString(36).substr(2,8);"
"item.innerHTML='<div class=\"wifi-name\">'+lock+' '+esc(ap.ssid)+'</div>'"
"+'<div class=\"wifi-right\">'+rssiToBars(ap.rssi)+'<span class=\"rssi\">'+ap.rssi+'dBm</span>'"
"+'<button class=\"btn-copy\" onclick=\"event.stopPropagation();copySSID(\''+ap.ssid.replace(/\\/g,'\\\\').replace(/'/g,'\\\\\'')+'\')\">\u590D\u5236;</button></div>';"
""
...
"var res=await fetch('/connect',{method:'POST',body:fd});"
"var txt=await res.text();"
"if(txt==='ok'){"
"showStatus('success','\u8FDE\u63A5\u6210\u529F;\uFF01\u8BBE\u5907;\u5C06;\u91CD;\u542F;\u5E76;\u8FDE;\u63A5;\u5230;\u7F51;\u7EDC;...');"
"}else{"
"showStatus('error','\u8FDE\u63A5;\u5931;\u8D25;\uFF0C\u8BF7;\u68C0;\u67E5;\u5BC6;\u7801;\u662F;\u5426;\u6B63;\u786E;');"
"btn.disabled=false;"
"}"
"}catch(e){"
"showStatus('error','\u8FDE\u63A5;\u8BF7;\u6C42;\u5931;\u8D25;');"
"btn.disabled=false;"
"}"
"}"
